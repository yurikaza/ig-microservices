 ...
 */
export async function publish(revision, key, service = defaultService) {
    const url = new URL(`name/${revision.name.toString()}`, service.endpoint);
    const entry = await ipns.create(key, uint8ArrayFromString(revision.value), revision.sequence, new Date(revision.validity).getTime() - Date.now());
 ...
 */
export async function resolve(name, service = defaultService) {
    ...
    return new Revision(name, uint8ArrayToString(entry.value), entry.sequence, uint8ArrayToString(entry.validity));
}
